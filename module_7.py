...
# We have copied the live_plotter.py from PythonClient to current dir
import live_plotter as lv  # Custom live plotting library

# To solve repeated name problem between official carla package and the package in Pythonclient, we rename it to be carl_lib
from carla_lib import sensor
from carla_lib.client import make_carla_client, VehicleControl
from carla_lib.settings import CarlaSettings
from carla_lib.tcp import TCPConnectionError
from carla_lib.controller import utils
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        config.read(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "options.cfg")
        )
        ...
        with open(waypoints_file) as waypoints_file_handle:
            waypoints = list(
                csv.reader(
                    waypoints_file_handle, delimiter=",", quoting=csv.QUOTE_NONNUMERIC
                )
            )
        ...
        for i in range(num_iterations):
            ...
            if i == num_iterations - 1:
                sim_duration = (
                    measurement_data.game_timestamp / 1000.0 - sim_start_stamp
                )
        ...
        forward_speed_fig.add_graph(
            "forward_speed", label="forward_speed", window_size=TOTAL_EPISODE_FRAMES
        )
        forward_speed_fig.add_graph(
            "reference_signal",
        ...
        throttle_fig.add_graph(
            "throttle", label="throttle", window_size=TOTAL_EPISODE_FRAMES
        )
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            if skip_first_frame and frame == 0:
                ...
                path_indices = np.floor(
                    np.linspace(
                        0, new_waypoints_np.shape[0] - 1, INTERP_MAX_POINTS_PLOT
                    )
                )
                ...
                forward_speed_fig.roll(
                    "forward_speed", current_timestamp, current_speed
                )
                forward_speed_fig.roll(
            ...
            send_control_command(
                client, throttle=cmd_throttle, steer=cmd_steer, brake=cmd_brake
            )
